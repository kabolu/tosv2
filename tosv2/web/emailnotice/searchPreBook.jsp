...
function sendEmail(){
	...
	document.location.href = "EmailNoticeServlet?type=PREBOOK&studentNos="+studentNos+"&orderSeqNos="+orderSeqNos+"&isbn="+isbn+"&isbnSeqNos="+isbnSeqNos+"";
}
...
function checkForm(){
    ...
    var c = document.getElementById("studentId").value;
    if(a != "" && b != ""){
...
<body>
  <h2>預訂教材通知</h2>
  	<form action="StudentLackBookServlet" method="post" name="searchForm" onsubmit="return checkForm();">
  	  <input type="hidden" name="type" value="SEARCHPREBOOK"/>
  	  <table border="0">
  	    <tr>
  	      <td>訂書日期：</td>
  	      <td>
  	    ...
  	    <tr>
  	      <td>學生編號：</td>
  	      <td>
  	        <input id="studentId" type="text" class="inp" name="studentNo" />
  	      </td>
  	    </tr>
  	    <tr>
	...
	for(int i=0; i<size; i++){
		...
		<tr onmouseover="this.bgColor='#FFFFFF'" onmouseout="this.bgColor='#FFFBEF'" bgcolor="#FFFBEF" >
    	  <td align="center">
    	    <%
    	    String tmpStudentNo = "";
    	    if(!"".equals(stud.getStudentNo()) && stud.getStudentNo() != null){
    	    	tmpStudentNo = stud.getStudentNo();
    	    } else {
    	    	tmpStudentNo = stud.getApplicantNo();
    	    }    	    	
    	    %>
    	    <input type="checkbox" name="emailbox" value="<%=tmpStudentNo %>" class="checkbox"/>
    	    <input type="hidden" name="orderSeqNo" value="<%=od.getOrderSeqNo() %>" />
    	    <input type="hidden" name="isbnSeqNo" value="<%=od.getIsbn() %>" />
    	  </td>
    	  <td height="25">
    	    <a href="#"><span onclick="openDetail('StudentDetailServlet?psStudentNo=<%=tmpStudentNo %>','','width=700,height=400,toolbar=no,status=no,menubar=no,scrollbars=no,resizable=no')">
    	    <%=tmpStudentNo %>
    	    </span></a>
    	  </td>
