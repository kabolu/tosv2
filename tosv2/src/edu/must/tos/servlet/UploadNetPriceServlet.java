...
public class UploadNetPriceServlet extends HttpServlet {

	/**
	 * Constructor of the object.
	 */
	public UploadNetPriceServlet() {
		super();
	}
			...
			if ( ServletFileUpload.isMultipartContent(requestContext) ) {
				...
				for ( int i = 0 ; i < items.size(); i++ ) {
					...
					if( !"xls".equalsIgnoreCase(extName) ) {
						...
						request.setAttribute("type", "uploadPrice");
 				...
 				if(format){
 	                ...
 	                List errIsbnFormat = getCheckIsbnFormat(fileItem);
 	 				if( !resultErrYearAndPrice.isEmpty() || !errIsbnFormat.isEmpty()){
 	 					...
 	 					int temp = priceImpl.getUpdateNetPrice(conn, price);
 	 					if(temp > 0){
 	 						conn.commit();
 	 						request.setAttribute("msg", "Books price update success!");
 	 						request.setAttribute("type", "uploadPrice");
 	 						request.getRequestDispatcher("msg.jsp").forward(request, response);
 	 					}else{
 	 						conn.rollback();
 	 						request.setAttribute("msg", "Books price update faile!");
 	 						request.setAttribute("type", "uploadPrice");
 	 						request.getRequestDispatcher("msg.jsp").forward(request, response);
 	 					}
 	 				}
            	...
            	if ( sheet.getCell(j,a).getContents() == null || sheet.getCell(j,a).getContents().equals("") ) {
     			...
     			for(int k=0; k<2; k++){
         			...
         			if ( k == 0 )
         				...
         				p1.setCurrency("RMB");
         			p1.setNetPrice(new Double(sheet.getCell(k+6, a).getContents().trim().replace("\\n\\r", "")));
         			p1.setCreUid(userId);
         			p1.setUpdUid(userId);
         			p1.setActInd("Y");
         			p1.setIntake(intake);
